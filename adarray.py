...
@append_docstring_from_numpy
def tile(*args, **kargs):
    return array(np.tile(*args, **kargs))
...
def erf(x, out=None):
    x = array(x)
    out = adarray(math.erf(x._value))

    multiplier = dia_jac( 2./np.sqrt(np.pi) * np.exp(-( np.ravel(x._value) )**2) )
    out.next_state(multiplier, x, 'erf')

    if __DEBUG_MODE__:
        out._DEBUG_perturb = _DEBUG_perturb_retrieve(x) \
                           * 2./np.sqrt(np.pi) * np.exp(-(x._value)**2)
        _DEBUG_perturb_verify(out)
    return out
